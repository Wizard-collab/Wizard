...
from gui.ref_list_widget import Ui_Form
...
class Main(QtWidgets.QWidget):
    ...
    def add_reference(self, asset=None, selectable=True):
        if asset:
            ...
            count = asset[1]
            asset = asset[0]
            widget = ref_list_asset_widget.Main(self, asset, count, selectable, proxy, visible)
            if self.references > 5:
                self.references = 0
                self.row += 1
            if asset.export_version:
                self.ui.ref_assets_verticalLayout.addWidget(widget)
                self.references += 1
    ...
    def refresh_references(self, asset=None):
        for i in reversed(range(self.ui.ref_assets_verticalLayout.count())):
            self.ui.ref_assets_verticalLayout.itemAt(i).widget().setParent(None)
        if asset:
            if asset.stage:
